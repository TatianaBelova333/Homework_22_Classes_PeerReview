...
from assets import Shop, Storehouse, Request, UserRequestError, StorageFull


def create_instances(goods_shop: List[Tuple[str, int]], goods_store: List[Tuple[str, int]]) -> [Shop, Storehouse]:
...
def display_items(shop: Shop, store: Storehouse) -> str:
...
def send_request(user_task: str, shop: Shop, store: Storehouse) -> None:
    """Send request and return fulfillment process"""
    if user_task.lower() == 'стоп':
        exit()
    try:
        request = Request(storages={'магазин': shop, 'склад': store}, user_request=user_task)
        request.process()
    ...
    except (UserRequestError, StorageFull, ItemsNotFound) as e:
        print(e)
